 ...
 * Time: 9:50 AM
 */
class JobSaleMoneyController extends SiteController
{
    ...
    public function create_employee_share_job(Request $request)
    {
        try {
            DB::beginTransaction();
            ...
            {
                $total_sale_job = $employee_coints->total_sale_job + 1;
                $update = $employee_coins_model->where('employee_id', $employee_id)->update([
                    'total_sale_job' => $total_sale_job,
                    'updated_at' => new \DateTime()
                ]);
            }
//            thống kê chia sẻ bài viết theo ứng viên
            $post_sale_static = new Job_sale_statistical();
...
//            nếu tồn tại thì cộng số lần chia sẻ ứng viên
    ...
    public function add_ajax_sale_money_job(Request $request)
    {
        $ip_sale = !empty($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 0;
        if (!empty($request->input('job_id')) && !empty($request->input('employee_id')) && !empty($ip_sale)) {
            DB::beginTransaction();
            ...
            $job = new Job();
            $total_job = $job->select('job_id', 'sale_money','deadline_submit_profile')->where('job_id', $job_id)
...
//            check tin tuyển dụng hết hạn hoặc dừng trạng thái chia sẻ
            ...
            if ($total_job <= 0) {
                return response([
                    ...
                    'message' => 'tin tuyển dụng này đã hết hạn hoặc không được chia sẻ hoạc đã dừng'
                ],404)->header('Content-Type', 'text/plain');
            }
...
//                ->where('job_id', '=', $job_id)
            ...
            if ($total_sale > 0) {
                ...
                //tao lượt xem
                $this->create_post_sale_job($employee_id, $job_id, $ip_sale, $day_date);
				  DB::commit();
				return response([
                'status' => 200,
    ...
    public function create_post_sale_job($employee_id, $job_id, $ip_sale, $day_date)
    {
...
//        $money_view = !empty($informationShow_money['so-tien-luot-xem-tin-tuyen-dung']) ? $informationShow_money['so-tien-luot-xem-tin-tuyen-dung'] : 150;
//        //Số tiền tối đa nhận được khi chia sẻ bài viết
//        $max_view_post = !empty($informationShow_money['so-tien-toi-da-trong-1-tin-tuyen-dung']) ? $informationShow_money['so-tien-toi-da-trong-1-tin-tuyen-dung'] : 150000;
        ...
        if (!empty($check_post_sale_static)) {
            ...
            if($total_view_sale % 10 == 0)
            {
                ...
                $noti_title = 'Bạn được cộng thêm 1 xu từ chia sẻ tin tuyển dụng trên sanketoan.vn';
    ...
    public function delete_post_sale_money_job()
    {
        try{
            $day_date = new \DateTime();
            $post_sale_model = new Job_sale_money();
...
//                print_r($check_sale);die();
    ...
    public function get_total_sale_money_employer(Request $request)
    {
        $employee_coin = Employee_coins::get();
        foreach($employee_coin as $coin)
        {
            $total_share = Job_sale_statistical::where('employee_id',$coin->employee_id)->sum('total_share');
            $total_view_sale = Job_sale_statistical::where('employee_id',$coin->employee_id)->sum('total_view_sale');

            $update_employee_coin = Employee_coins::where('employee_id',$coin->employee_id)->update([
                'total_sale_job' => $total_share, //Tống số lần chia sẻ tin tuyển dụng
                'total_view_job' => $total_view_sale //Tống số lần xem tin tuyển dụng
            ]);
            echo 'employee:'.$coin->employee_id.'--'.'share:'.$total_share.'--'.'total_view_sale'.$total_view_sale.'</br>';
        }
    }
