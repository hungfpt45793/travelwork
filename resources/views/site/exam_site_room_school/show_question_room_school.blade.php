...
<section class="main" id="toTop">
    <div class="container">
        <form action="{{ route('createSchoolResultDetailRoom')}}" method="POST" id="submitQuenstionRoom">
            {!! csrf_field() !!}
            {{ method_field('POST') }}
            <input type="hidden" name="id_exam" value="{{ $exam['id_exam'] }}">
            <input type="hidden" name="id_room" value="{{ $room['id_room'] }}">
            <input type="hidden" name="id_result" value="{{ $result->id_result }}">
                            ...
                            <div id="questionlist" data-exam-id="27" class="m-bottom">
                                @foreach($questions as $id => $question)
                                    <div id="question_{{ $id + 1 }}" id_data="question_number_{{ $id + 1 }}"
                                        ...
                                        <div class="panel panel-default  @if($question['type_ques'] == 3) question-item2 @else question-item @endif"
...
@endif
                                ...
                                <div class="text-center clear">
                                    ...
                                    <button id="ShowAllQuestion" type="button" class="btn bgHome clwhite item-next mgb5"
                                            title="Kế tiếp" style="margin-bottom: 5px">
                                        <span class="btn-label">Hiển thị tất cả câu hỏi</span>
                                    </button>

                                    <button onclick="previous_question();" type="button"
                                            class="btn btn-danger item-prev mgb5" title="Trước đó" disabled="disabled"  style="margin-bottom: 5px">
                                        <span>Câu trước đó</span>
                                    </button>
                                    &nbsp;
                                    <button onclick="next_question();" type="button"
                                            class="btn btn-primary item-next mgb5" title="Kế tiếp"  style="margin-bottom: 5px">
                        ...
                        <ul class="slides-vertical-pagination">
                            @foreach($questions as $id => $question )
                                <li class="question_number" id="question_number_{{$id + 1}}"
                                    onclick="scrolltoQuestion({{$id + 1}}),ser_process_bar({{$id + 1}}, {{ $countQuestion }})">
                                    <a href="#toTop"><span
...
<section>
    ...
    <script>
        $(document).ready(function () {
            function myFunction() {
                document.getElementById("submitQuenstionRoom").reset();
            }
...
<div class="modal fade" id="exam_rules" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                {!! isset($room['exam_rules']) ? $room['exam_rules']  : '' !!}
            </div>
...
<script>
    var time = <?php echo $exam['time_exam']; ?> * 60;
    ...
    var counter = function () {
        if (value <= 0) {
            ...
            setTimeout(function(){
                ...
                $("#btn_submit_exam").attr("disabled", true);
                $('#submitQuenstionRoom').submit();
                $('#submitQuenstionRoom').submit(function(event){
