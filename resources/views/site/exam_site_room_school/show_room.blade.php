...
@section('content')
    ...
    border-radius: 0;">
                    ...
                    <div class="titleJobs textUpper fw7 f18 white bgrBlueN pd10-20 col-f14 mgt30">
                        Danh sách phòng thi do giảng viên tạo
                    </div>
                    <div class="categoryQuestion">
                        <div class="main">
                            <div class="notificationBox bkwhite formJobLarge sm-f14">
                                <div class="bodyBox ">
                                    <div class="row ListRoom">
                                        ...
                                        @if(!empty($list_room_school))
                                            @foreach($list_room_school as $room)
                                                <div class="col-lg-6 col-md-6 col-12">
    ...
    @foreach($list_room_school as $room)
        <div class="modal fade" id="room_student{{ $room->id_room }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="{{ route('submitRoomPassword') }}" method="POST">
                        {!! csrf_field() !!}
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Vui lòng nhập thông tin sinh viên để vào thi</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            ...
                            <div class="form-group mgb0">
                                ...
                                <input type="text" class="form-control" name="student_name" placeholder="Tên sinh viên ...">
                            </div>
                            <div class="form-group mgb0">
                                <label for="recipient-name" class="col-form-label">Email sinh viên</label>
                                <input type="email" class="form-control" name="student_email" placeholder="Email sinh viên ...">
                                <i class="f12">Vui lòng nhập đúng địa chỉ email để nhận kết quả bài thi về email của bạn</i>
                            </div>
                            <div class="form-group mgb0">
                                <label for="recipient-name" class="col-form-label">Số điện thoại sinh viên</label>
                                <input type="number" class="form-control" name="student_phone" placeholder="Số điện thoại sinh viên ...">
                            </div>
                            <div class="form-group mgb0">
                                @if(session('erorrRoomPassword'))
                                    <p style="color: red;margin-bottom: 0">{{ session('erorrRoomPassword') }} </p>
