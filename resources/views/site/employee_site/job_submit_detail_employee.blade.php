...
@section('show_css')
    ...
    <link rel="stylesheet" type="text/css" href="{{ asset('assets/web/css/detail_employee.css') }}"/>
    <link rel="stylesheet" type="text/css" href="{{ asset('assets/web/css/preview_pdf.css') }}"/>
...
@section('content')
        ...
        <div class="container-fluid ">
            <div class="row">
                <div class="col-md-12 mgb10">
                    <div class="text-center" style="border: 1px solid #ccc;background: #fff;padding: 20px">
                        <h3 class="f22 fw6 clgreen">Đơn xin việc</h3>
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                        ...
                        2)
                            ...
                            @if(!empty($check_show_employee))
                                {{--<h3 class="f20 fw6 clGreen">Thông tin liên hệ</h3>--}}
                                    ...
                                    <h3 class="f20 fw6 clGreen dsInline" style="display: inline-block">Thông tin liên hệ</h3> <a href="{{ $link_cv_upload }}" download="{{ !empty($employee->employee_slug) ? $employee->employee_slug : '' }}" class="detail_dowload_cv">Tải xuống CV <i class="fas fa-file-download"></i></a>
                    ...
                    @endif
                    <!--{{--//trạng thái hồ sơ--}}-->
                        <form class="d-flex justify-content-center" action="{{ route('save_id_status_submit_job') }}"
                              method="post">
                            <table class="table table-bordered table_info mgb5">
                                <tbody class="info_contact">
                                <tr>
                                    <td><span class="clRed fw6">Trạng thái HS</span></td>
                                    <td>
                                        <select class="custom-select form-control form-control-sm js_change_select"
                                                name="id_status_submit_job">
                                            <option value="0"
                                                    @if($submit_job_employee->id_status_submit_job == 0) selected @endif>
                                                Trạng thái hồ sơ
                                            </option>
                                            <?php
                                            ...
                                            @foreach($list_status as $status)
                                                <option @if($submit_job_employee->id_status_submit_job == $status->id_status) selected
                                                        @endif  value="{{ isset($status->id_status) ? $status->id_status : '' }}">
                                                    {{ isset($status->name_status) ? $status->name_status : '' }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <input type="hidden" name="submit_job_fb_id"
                                               value="{{ $submit_job_employee->submit_job_fb_id }}">
                                        <input type="hidden" name="status_job"
                                               value="{{ $submit_job_employee->status_job }}">
                                    </td>
                                    <td>
                                        <button type="submit"
                                                class="btn btn-sm btn-outline-danger submit_show_info_cv_detail_employee">
                        ...
                        <ul class="list-group ul_action">
                            <li class="list-group-item cus-list-group-item dsBlock">
                                <span class="btn_send_employee"> <i class="fas fa-hand-spock text-success"></i> <a
                                            class="js_apply_employee_job a_cursor "> Mời ứng tuyển </a> </span>
                            </li>
                            <li class="list-group-item cus-list-group-item dsBlock">
                                <span class="btn_ratting_employee"> <i class="fas fa-star text-warning"></i> <span
                                            type="button" class="js_vote_employee a_cursor"
                                            data-employee-id="{{ $employee->employee_id }}"> Đánh giá ứng viên </span> </span>

                            </li>
                            <li class="list-group-item cus-list-group-item dsBlock">

                                <span class="btn_feedback_employee"> <i class="fas fa-reply-all text-success"></i> <span
                                            type="button" class="js_response_employee a_cursor"
                                            data-employee-id="{{ $employee->employee_id }}"> Phản hồi chất lượng CV </span> </span>
                            </li>
                        </ul>
                ...
                <form action="{{ route('send_job_employer') }}" method="post">
                    ...
                    <div class="modal-content">
                            ...
                            @foreach($list_apply_job as $apply_job)
                                ...
                                @if(!empty($check_employee_apply))
                                    <label class="clGreen mgb0">
                                        ...
                                        {{ $apply_job->title }}
                                    </label> <sup>({{ !empty($apply_job->view_apply) ? $apply_job->view_apply : '' }}
                                        điểm)</sup>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="box_btn_submit_profile btn btn-primary send_evaluate">Mời ứng
                                tuyển
                            </button>
                        </div>
                    </div>
    ...
    <div id="vote_employee" class="modal fade" role="dialog">
        <div class="modal-dialog">
            <form action="{{ route('vote_employee') }}" method="post">
                {!! csrf_field() !!}
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Đóng</button>
                        <button type="submit" class="box_btn_submit_profile  btn btn-primary send_evaluate">Đánh giá
                        </button>
                    </div>
                </div>
            </form>
