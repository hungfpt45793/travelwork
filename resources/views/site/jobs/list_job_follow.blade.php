...
@section('title', 'Việc làm theo dõi từ nhà tuyển dụng')
                ...
                <div class="col-xl-9 col-lg-8 col-md-12 createProfileOnline ">
                    <div class="link bgrWhite md-mgt20">
                        <ul class="nav">
                            ...
                            <li class="nav-item pd8">
                                <a href="#" class=" f18 md-f14 mgb0">Việc làm của nhà tuyển dụng</a>
                            </li>

                        </ul>
                    </div>
                    @if(!$list_follow_employer->isEmpty())
                    <div class="link bgrWhite mgt20 pd10 list_follew_employer">
                      @foreach($list_follow_employer  as $follow)
                          <?php
                            $total_job_follow = 0;
                            $total_job_follow = \App\Entity\Job::total_job_employer($follow->employer_id);
                            ?>
                        <span class="js_submit_form @if(!empty($id_employer) && $id_employer == $follow->employer_id) active @endif">
                           <a href="{{ route('list_employee_follow_employer_id',['employer_d'=> $follow->employer_id]) }}">{{ $follow->enterprise_name }}
                               @if(!empty($total_job_follow))
                               <sup class="clred">({{ $total_job_follow }})</sup>
                                   @endif
                           </a>
                            <i id_employer="{{ $follow->employer_id }}" class="fas fa-times js_delete_follow_employer" style="cursor: pointer"></i>
                        </span>
                          @endforeach
                    </div>
                    @endif
                    ...
                    <section class="jobsInteresting bgrWhite  radius5 mgt20">
                        <div class="titleJobs  fw6 f20 white bgrBlueN pd10-20 col-f14">
                           Theo dõi việc làm từ các nhà tuyển dụng
                        </div>
