<div class="col-xl-6 col-lg-6 pd0 bdBottomGray bdRightGray hvbgrClick">
    <div class="JobInteresting pd hvBoxShadow">
        ...
        <div class="company @if($job->vip == 1) newVip @endif">
            <a href="{{ route('job_detail',['slug' => $job->slug ]) }}" class="block pd15 noDecoration CutText100" title="{{ $job['title'] }}">
         ...
         {{ isset($job['title']) ? \App\Ultility\Ultility::textLimit($job['title'], 15) : '' }}
             ...
             @if(!empty($image) && $image = 'job')
                 <img class="lazy" class="chuaxathuc chuaxacthucItemJob" src="{{ asset('assets/image/xacthuc.jpg') }}"
         ...
         Lương:
                ...
                <a href="{{ route('job_detail',['slug' => $job->slug ]) }}" class="link" style="margin-top: 0;
    margin-left: 10px;
    margin-bottom: 10px;" title="{{ $job['title'] }}">Xem thêm</a>
