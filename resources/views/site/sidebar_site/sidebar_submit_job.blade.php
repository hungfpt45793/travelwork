...
<div class="dnav col-xl-3 col-lg-4 col-md-12 sidebar_job active_show_sidebar" id="js_toogle_sidebar">
    <div class="d-toggle">
        ...
        <div class="fillterJobSubmit text-left">
            ...
            <ul class="ul_list_job_submit text-left">
                @if(!empty($list))
                    @foreach($list as $l)
                        <li class="mgb10 @if($l->job_id == $job_id) li_focus @endif">
                            <a href="{{ route('job_Candidate_Employee',['job_id'=>$l->job_id]) }}" class="f16">
                                <i class="far fa-folder-open mgr5 f16 "></i>
                                {{ isset($l->title) ? $l->title : '' }} -
                                <span class="code_job">{{ isset($l->job_code) ? $l->job_code : '' }}</span>
                            </a>
                        </li>
                    @endforeach
                @else
            ...
            ?>
            <form class="form_filter_submit_job" action="" method="get">
                <div class="">
                    <label class="f16 dnavnone">Trạng thái hồ sơ</label>
                </div>
                <?php
                $list_status_submit = \App\Entity\Status_submit_job::get_list(10);
                ?>
                @if(!empty($list_status_submit ))
                    <div class="dsBlock">
                        ...
                        <label class="f16">
                            ...
                            <?php
                            $count_status = 0;
                            $count_status = \App\Entity\Employee_submit_job_faacebook::getTotalStatusJobCV($job_id,0);
                            ?>
                    ...
                    @foreach($list_status_submit as $status_submit)
                        <div class="dsBlock">
                            <?php
                            $count_status = 0;
                            $count_status = \App\Entity\Employee_submit_job_faacebook::getTotalStatusJobCV($job_id,$status_submit->id_status);
                            ?>
                            <label class="f16">
                    ...
                    @endforeach
                    <div class="dsBlock">
                        <button  data-toggle="tooltip" data-placement="right" title="Lọc hồ sơ" type="submit" id="btnloading_frofile"><i class="fas fa-filter"></i><span class="dnavnone">  Lọc hồ sơ</span></button>
                    </div>
                @endif
            </form>

            <div>
                <a data-toggle="tooltip" data-placement="right" title="Quay về tủ hồ sơ" href="{{ route('list_job_face') }}" class="dsBlock  f18 clHome text-center"><i class="fas fa-long-arrow-alt-left"></i><span class="dnavnone"> Quay về  tủ hồ sơ  <i class="fas fa-long-arrow-alt-right"></i></span></a>
            </div>
