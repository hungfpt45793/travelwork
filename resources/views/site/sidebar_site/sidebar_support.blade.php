    ...
    <div class="UvNew bgrWhite">
        ...
        <div class="contentsUvNew bdLightGray">
            ...
            @foreach (\App\Entity\Post::relativeProduct($post->slug,10) as $id => $post_relati)
                <a href="{{ route('post', ['cate_slug' => $cate_slug, 'post_slug' => $post_relati->slug]) }}" class="NoDecoration">
                    <div class="pd10 pdl20 pdr20 bdBottomGray hvbgrClick ">
                        <h3 class="textCap mgb0 black CutTextW250 f14"><i class="fas fa-caret-right"></i> {{ isset($post_relati->title) ? \App\Ultility\Ultility::textLimit($post_relati->title, 12) : '' }}</h3>
                        <p style="margin-bottom: 0" class="f14">{{ isset($post_relati->description) ? \App\Ultility\Ultility::textLimit($post_relati->description, 30) : '' }}</p>
                    </div>
                </a>
            @endforeach
