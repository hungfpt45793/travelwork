...
@section('meta_image', !empty($employee->employee_image) ? asset($employee->employee_image) : asset($information['logo']) )
                ...
                <div class="col-lg-8 div_width pr-0" style="background-color: #fff;position:relative" id="">
                    ...
                    @else
                        ...
                        <div id="appendToThis">
                            @if(!empty($check_show_employee))
                                <?php
                                $link_cv_upload = route('employer_exportpdf_cv_user_id', ['user_id' => $employee->user_id]).'?employer_id='.$employer->employer_id;
                                ?>
                                <iframe class="iframe_cv_employee" src="https://docs.google.com/gview?url={{ asset($link_cv_upload) }}&embedded=true" frameborder="0"></iframe>
                            @else
                ...
                <div class="col-lg-4">
                    <div class="content_left_info ">
                        ...
                        <table class="js_info_different table table-bordered table_info mb-0">
                            ...
                            <tr>
                                <td>Chi phí từ</td>
                                <td colspan="2"><span style="color: red;font-weight: bold">{{ !empty($comco_ad->combo_price) ? number_format($comco_ad->combo_price ) : '0' }} VNĐ</span></td>
                            ...
                            <tr>
                                <td>Kết nối</td>
                                <td colspan="2"> <?php
                                    $check_advise = \App\Entity\User_advise::where('user_id', $employee->user_id)
                                        ->where('ad_status', 1)
                                        ->first();
                                    ?>
                                    @if(\Illuminate\Support\Facades\Auth::check())
                                        <a class="advise_connect"
                                           href="{{ route('get_connect_user_support',['user_id'=>$employee->user_id]) }}"><i
                        ...
                        <div class="modal-body">
                            <div class="content_box_res_advise">
                                ...
                                <div class="text-center">
                                    ...
                                    <button type="submit" class="btn btn-primary" src="">Kết nối với gia sư</button>
                                </div>
                            </div>
