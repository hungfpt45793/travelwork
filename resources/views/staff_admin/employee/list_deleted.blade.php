                ...
                <div class="contentJobsInteresting  col-f14 ">
                    ...
                    <div class="custom-order">
                        ...
                        <div class="d-flex justify-content-start second-order">
                            ...
                            <a href="{{ route('staff_employee_list_deleted') }}" class="btn btn-sm btn-secondary mr-1 text-white"><i class="fas fa-sync-alt text-success"></i> Làm tươi</a>
                                ...
                                <table data-fl-scrolls class="custom-table table-scroll table-bordered table-striped" style="overflow: scroll;height:100vh;display:block;table-layout:fixed;">
                                    <thead>
                                        <tr>
                                            ...
                                            <td scope="col" class="lid_9"><p style="width:80px">ĐN xóa<button class="lockButton btn btn-sm btn-success" id="lid_9">L</button></p></td>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td scope="row" class="lid_1 val" data-val="{{ $employee->employee_id }}">{{ $employee->employee_id }}</td>
                                            <td class="lid_2">
                                                <a href="{{ route('delete_employee_hard',$employee->employee_id) }}" class="btn btn-sm btn-danger" onclick="confirm('Bạn chắc chắn muốn xóa!')">Xóa</a>
                                                <a href="{{ route('reset_employee',$employee->employee_id) }}" class="btn btn-sm btn-success">RS</a>
                                            </td>
                                            <td class="lid_3">
                                                <?php
                                                $date=date_create($employee->deleted_at);
                                            ...
                                            <td class="lid_9">
                                                <?php
                                                $check_delete = \App\Entity\Employee_delete_request::where('employee_id',$employee->employee_id)->first();
                                                ?>
        ...
        $('.xem_spec').click(function(){
            ...
            $.ajax({
                ...
                success: function (data) {
                    var specialize = data.specialize;
        ...
        $('.delete_all').click(function(){
            ...
            if (x){
                ...
                if(Ids.length == 0){
                    ...
                    $.ajax({
                        type: 'post',
                        url: '{{route("delete_allemployee_hard")}}',
                        data: 'Ids='+Ids,
                        success: function (data) {
                            if (data) {
                                var cut = data.split(",");
                                $("table tr").each(function(index, tr) {
                                    if(index > 0) {
                                        let employee_id = tr.querySelector('td').getAttribute('data-val');
                                        cut.forEach(element => {
