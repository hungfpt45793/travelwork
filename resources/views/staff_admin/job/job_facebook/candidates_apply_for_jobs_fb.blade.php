                ...
                <div class="contentJobsInteresting  col-f14 ">
                    ...
                    <div class="custom-order">
                        <div class="custom-paginate first-order ml-1 mt-1 row">
                            ...
                            | xem 1-{{ isset($_GET['num']) ? $_GET['num'] : '30' }}/{{ $job_facebook->total() }} bản ghi
                        </div>
                        <div class="d-flex justify-content-between second-order" style="width:-webkit-fill-available">
                                                        ...
                                                        <div class="form-group row">
                                                            ...
                                                            <div class="col-md-9">
                                                                <div class="input-group">
                                                                    <input type="text" class="form-control" value="{{ isset($_GET['job_facebook_code']) ? $_GET['job_facebook_code'] : '' }}" name="job_facebook_code" placeholder="Mã tin..." >
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="form-group row">
                            ...
                            <a href="{{ route('candidates_apply_for_jobs_fb') }}" class="btn btn-sm btn-secondary mr-1 text-white"><i class="fas fa-sync-alt text-success"></i> Làm tươi</a>
                        ...
                        <div class="col-md-12">
                            <div id="locker" data-fl-scrolls class="custom-table table-bordered table-striped" style="overflow: scroll;height:100vh;display:block;table-layout:fixed;">
                                <div class="lockedWrap lockedWrap-first">
                                    ...
                                    @foreach ($job_facebook as $employee)
                                    <div class="cellWrap">
                                ...
                                <table  data-fl-scrolls class="custom-table table-scroll table-bordered table-striped" style="overflow: scroll;height:100vh;display:block;table-layout:fixed;">
                                    <thead>
                                        <tr>
                                            ...
                                            <td scope="col" class="lid_4"><p style="width:145px">Tên tin tuyển dụng<button class="lockButton btn btn-sm btn-success" id="lid_2">L</button></p></td>
                                            ...
                                            <td scope="col" class="lid_7"><p style="width:150px">Tên nhà tuyển dụng<button class="lockButton btn btn-sm btn-success" id="lid_6">L</button></p></td>
                                    ...
                                    <tbody>
                                        @foreach ($job_facebook as $job_fb)
                                        <tr>
                                            <td class="lid_1">{{ $job_fb->job_facebook_id }}</td>
                                            <td class="lid_1">{{ $job_fb->job_facebook_code }}</td>
                                            <td class="lid_1 crop">{{ $job_fb->title }}</td>
                                            <td class="lid_1 text-center">{{ $job_fb->view }}</td>
                                            <td class="lid_5">
                                                <p class="crop">
                                                    <a target="_blank" href="{{ route('staff_employee.index') }}?job_facebook_id={{$job_fb->job_facebook_id}}" class="text-primary">
                                                        @php
                                                            $UV = \App\Entity\Employee_submit_job_faacebook::where('id_job_fb', $job_fb->job_facebook_id)->where('status_job', 0)->count();
                                            ...
                                            <td class="lid_5 text-center">
                                                @php
                                                    $date = date_create($job_fb->update_at);
                                                    echo date_format($date, "d/m/Y");
                                                @endphp
                                            </td>
                                            <td class="lid_5 text-center">
                                                @php
                                                    $date = date_create($job_fb->date_end);
                                                    echo date_format($date, "d/m/Y");
                                                @endphp
                                            </td>
                                            ...
                                            <td class="lid_1 crop">{{ $job_fb->enterprise_name }}</td>
                                            <td class="lid_1 text-center">{{ $job_fb->phone }}</td>
