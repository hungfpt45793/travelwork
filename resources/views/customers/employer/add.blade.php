...
{{--<script src="https://unpkg.com/axios/dist/axios.min.js"></script>--}}
    ...
    <section class="content">
        <div class="row">
            <!-- form start -->
            <form role="form" action="{{ route('employer.store') }}" method="POST">
                {!! csrf_field() !!}
                {{ method_field('POST') }}
                    ...
                    <div class="box box-primary">
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="employer_name" placeholder="Tên doanh nghiệp" value="{{old('employer_name')}}" >
                            </div>
                            ...
                            <div class="form-group">
                                <label for="exampleInputEmail1">Password(đăng nhập)</label>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text"  id="location-input" class="form-control" name="address" placeholder="Địa chỉ chi tiết" value="{{old('address')}}" >
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="status" aria-label="Trạng thái">
                                    <option value="0" {{old('status') == 0 ? 'selected' : ''}}>Chưa có nhu cầu</option>
                                    <option value="1" {{old('status') == 1 ? 'selected' : ''}}>Có nhu cầu</option>
                                    <option value="2" {{old('status') == 2 ? 'selected' : ''}}>Đã lên đơn hàng</option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="exampleInputEmail1">Giới thiệu về công ty NTD</label>
                                <textarea class="editor" id="content" name="introduction" rows="10" cols="80"/>{{old('introduction')}}</textarea>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input name="images_list" type="hidden" value=""/>
                    ...
                    <div class="box box-primary boxCateScoll">
                        ...
                        <div class="box-body ">
                            <div class="form-group">
                                <label style="margin-right: 10px">
                                    <input type="radio" name="status_intership" value="0"
                                           class="flat-red" @if(old('status_intership') == 0)checked @endif>
                                    Không tuyển thực tập
                                </label>
                                <label>
                                    <input type="radio" name="status_intership" value="1"
                                           class="flat-red" @if(old('status_intership') == 1)checked @endif>
                                    Đang tuyển thực tập
                                </label>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="button" onclick="return uploadImage(this);" value="Chọn ảnh"
                                       size="20"/>
                                <img src="{{ old('banner_intership') }}" width="80" height=""/>
                                <input name="banner_intership" type="hidden" value="{{ old('banner_intership') }}"/>
                            </div>

                            <div class="form-group">
                                <label for="exampleInputEmail1">Mô tả thực tập</label><br>
                                <textarea rows="4" class="form-control" name="des_intership"
                                          placeholder="">{{ old('des_intership') }}</textarea>

                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Nội dung thực tập</label>
                                <textarea class="editor" id="content2_intership" name="content_intership" rows="10" cols="80"/>{!!
                                 old('content_intership') !!}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="box box-primary boxCateScoll">
                            ...
                            <div class="form-group">
                                <label style="margin-right: 10px">
                                    <input type="radio" name="employer_vip" value="0"
                                           class="flat-red" @if(old('employer_vip') == 0)checked @endif>
                                    Không Víp
                                </label>
                                <label>
                                    <input type="radio" name="employer_vip" value="1"
                                           class="flat-red" @if(old('employer_vip') == 1)checked @endif>
                                    Víp
                                </label>
                            </div>
                            ...
                            <div class="form-group">
                                <label style="margin-right: 10px">
                                    <input type="radio" name="status_allowance" value="0"
                                           class="flat-red" @if(old('status_allowance') == 0)checked @else checked @endif>
                                    Không có phụ câp
                                </label>
                                <label>
                                    <input type="radio" name="status_allowance" value="1"
                                           class="flat-red" @if(old('status_allowance') == 1)checked @endif>
                                    Có phụ cấp
                                </label>
                            </div>
                ...
                <div class="col-xs-12 col-md-4">
                    ...
                    <div class="form-group">
                        <label style="margin-right: 10px">
                            <input type="radio" name="status_agency" value="0"
                                   class="flat-red" checked>
                            Không phải là đại lý
                        </label>
                        <label>
                            <input type="radio" name="status_agency" value="1"
                                   class="flat-red" >
                            Đại lý
                        </label>
                    </div>
                    <div class="form-group">
                        <label style="margin-right: 10px">
                            <input type="radio" name="is_admin" value="0"
                                   class="flat-red" checked>
                         Mô tả dịch vụ đại lý
                        </label>
                        <textarea class="editor" id="service_agency" name="service_agency" rows="5" cols="80"/>{!!
                                 old('content_intership') !!}</textarea>
                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#type').keyup(function () {
                ...
                $.get('/admin/ajax-type/' + $(this).val(), function (data) {
                    $('#typeList').html(data);
                })
            });
            ...
            $('#business').keyup(function () {
                ...
                $.get('/admin/ajax-business/' + $(this).val(), function (data) {
