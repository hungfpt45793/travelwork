...
@section('title', 'Danh sách đổi thẻ cào' )

@section('content')
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>
            Danh sách đổi thẻ cào
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li><a href="#">Cài đặt</a></li>
            <li><a href="#">Danh sách đổi thẻ cào</a></li>
        </ol>
    </section>
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <table id="salaries" class="table table-bordered table-striped">
                            ...
                            <tbody>
                            @foreach($transaction_card  as $id=>$card)
                                <tr>
                                    <td>{{ $id + 1 }}</td>
                                    <td> {{ isset($card->employee_id) ? $card->employee_id : ''  }}
                                        - {{ isset($card->employee_name) ? $card->employee_name : ''  }}</td>
                                    <td> {{ isset($card->email) ? $card->email : ''  }}</td>
                                    <td> {{ isset($card->transaction_card_name) ? $card->transaction_card_name : ''  }}</td>
                                    <td> {{ isset($card->transaction_card_price) ? number_format($card->transaction_card_price) : ''  }}
                                        VND - {{ isset($card->transaction_total_coin) ? number_format($card->transaction_total_coin) : ''  }} xu
                                    </td>
                                    <td> {{ isset($card->transaction_card_phone) ? $card->transaction_card_phone : ''  }}</td>
                                    <td> {{ isset($card->transaction_content) ? $card->transaction_content : ''  }}</td>
                                    <td>
                                        @if($card->transaction_status == 0)
                                            <span style="background: orange;color: #fff;display: inline-block;padding: 3px 5px">Chưa giao dich</span>
                                        @endif
                                        @if($card->transaction_status == 1)
                                            <span style="background: red;color: #fff;display: inline-block;padding: 3px 5px">Đã hủy giao dịch</span>
                                        @endif
                                        @if($card->transaction_status == 2)
                                            <span style="background: green;color: #fff;display: inline-block;padding: 3px 5px">Đã giao dịch</span>
                                        @endif
                                    </td>
                                    <td>
                                        <?php
                                        $date = date_create($card->created_at);
                                        echo date_format($date, 'd/m/Y H:i:s');
                                        ?>
                                    </td>
                                    <td>
                                        @if(!empty($card->updated_at))
                                            <?php
                                            $date = date_create($card->updated_at);
                                            echo date_format($date, 'd/m/Y H:i:s');
                                            ?>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ route('transaction_card.edit',['transaction_card_id'=> $card->transaction_card_id]) }}">
                                            <button class="btn btn-primary"><i class="fa fa-pencil"
                                                                               aria-hidden="true"></i></button>
                                        </a>
                                        {{--<a href="{{ route('transaction_card.destroy',['transaction_card_id'=> $card->transaction_card_id]) }}"--}}
