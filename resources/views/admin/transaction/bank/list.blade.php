                    ...
                    <form role="search" method="GET" action="">
                        <div class="box-body">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="col-md-12">
                                        <label>Rút tiền từ App</label>
                                        <select class="form-control select2" name="transaction_total_coin">

                                            <?php
                                            $transaction_total_coin_get = '';

                                            if (isset($_GET['transaction_total_coin'])) {
                                                $transaction_status_get = $_GET['transaction_total_coin'];
                                            }
                                            ?>
                                            <option value="" selected>-- Trạng thái--</option>
                                            <option value="0"
                                                    @if($transaction_total_coin_get == '0') selected @endif
                                            >Không rút từ App
                                            </option>
                                            <option value="1"
                                                    @if($transaction_total_coin_get == '1') selected @endif
                                            >Rút từ App
                                            </option>
                        ...
                        <table id="salaries" class="table table-bordered table-striped">
                            ...
                            @foreach($transaction_bank  as $id=>$bank)
                                <tr>
                                    ...
                                    <td>
                                        {{ isset($bank->transaction_bank_name) ? $bank->transaction_bank_name : ''  }}
                                        <br>
                                       <span style="color: red"> {{ !empty($bank->transaction_total_coin) ? ' Rút xu từ App' : ''  }}</span>
                                    </td>
                                    <td> {{ isset($bank->transaction_bank_price) ? number_format($bank->transaction_bank_price) : ''  }}
                                        VND - {{ isset($bank->transaction_total_coin) ? number_format($bank->transaction_total_coin) : ''  }} xu
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('stop_trannsaction_bank',['employee_id'=>$bank->employee_id]) }}">  <span style="background: red;color: #fff;display: inline-block;padding: 3px 5px">Tạm dừng</span></a>
