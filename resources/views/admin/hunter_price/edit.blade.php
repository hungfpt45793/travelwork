...
<section class="content">
    <div class="row box">
        <div class="col-md-12">
            ...
            <form action="{{ route('hunter_price.update', $hunter_price->hunter_price_id) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                ...
                <div class="form-group">
                    ...
                    <select name="hunter_pos_id" id="hunter_pos_id" class="select2">
                        ...
                        @foreach (\App\Entity\Hunter_pos::get() as $item)
                            <option value="{{ $item->hunter_pos_id }}" {{ ($item->hunter_pos_id==$hunter_price->hunter_pos_id) ? 'selected' : '' }}>{{ $item->hunter_pos_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="hunter_time_id" id="hunter_time_id" class="select2">
                        ...
                        @foreach (\App\Entity\Hunter_time::get() as $item)
                            <option value="{{ $item->hunter_time_id }}" {{ ($item->hunter_time_id==$hunter_price->hunter_time_id) ? 'selected' : '' }}>{{ $item->hunter_time_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="hunter_price_name">Tiêu đề</label>
                    <input type="text" id="hunter_price_name" value="{{ old('hunter_price_name', $hunter_price->hunter_price_name) }}" name="hunter_price_name" class="form-control">
                </div>
                <div class="form-group">
                    <label for="hunter_price">Giá</label>
                    <input type="text" id="hunter_price" value="{{ old('hunter_price', $hunter_price->hunter_price) }}" name="hunter_price" class="form-control">
                </div>
