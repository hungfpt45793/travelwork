...
<section class="content">
    <div class="row box">
        <div class="col-md-12">
            ...
            <form action="{{ route('service_comment.update', $service_comment->service_comment_id) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div class="form-group">
                    <label for="service_comment_name">Tên người comment</label>
                    <input type="text" value="{{ old('service_comment_name',$service_comment->service_comment_name ?? '') }}" id="service_comment_name" name="service_comment_name" class="form-control">
                </div>
                <div class="form-group">
                    ...
                    <input name="service_comment_image" type="hidden"  value="{{ old('service_comment_image',$service_comment->service_comment_image ?? '') }}"  />
                </div>
                <div class="form-group">
                    <label for="service_comment_content">Nội dung comment</label>
                    <textarea id="service_comment_content" class="editor" name="service_comment_content" cols="80" rows="10">
                        {{ old('service_comment_content',$service_comment->service_comment_content ?? '') }}
                            </textarea>
                </div>
                <div class="form-group">
                    ...
                    <select name="service_price_id" class="select2" id="service_price_id">
                        ...
                        <option value="{{ $service_price->service_price_id }}"
                            {{ ($service_price->service_price_id==$service_comment->service_price_id) ? 'selected' : '' }}
                ...
                <div class="form-group">
                    ...
                    <select name="service_table_price_id" class="select2" id="service_table_price_id">
                        ...
                        <option value="{{ $service_table_price->service_table_price_id }}"
                            {{ ($service_table_price->service_table_price_id==$service_comment->service_table_price_id) ? 'selected' : '' }}
