...
@section('title', 'Danh sách kế toán cần hỗ trợ' )

@section('content')
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>
            Danh sách kế toán kết nối cần hỗ trợ
        </h1>
        <ol class="breadcrumb">
                        ...
                        <table id="salaries" class="table table-bordered table-striped">
                            ...
                            @foreach($list_ad  as $ad)
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                            $user_ad = \App\Entity\User_advise::where('ad_id',$ad->ad_id)->value('user_id');
                                        $user = \App\Entity\User::where('id',$user_ad)->first();
                                        ?>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal-xl{{$ad->ad_id}}"
                ...
                <form role="form" action="{{ route('update_advise_status') }}" method="POST">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <p>Trạng thái</p>
                            <label style="margin-right: 10px"><input type="radio" name="status_connect" value="0" @if($ad->status_connect == 0) checked @endif> Chưa xác nhận</label>
                            <label style="margin-right: 10px"><input type="radio" name="status_connect" value="1" @if($ad->status_connect == 1) checked @endif> Đã nhận</label>
                            <label style="margin-right: 10px"><input type="radio" name="status_connect" value="2" @if($ad->status_connect == 2) checked @endif> Từ chối</label>
                            <label style="margin-right: 10px"><input type="radio" name="status_connect" value="3" @if($ad->status_connect == 3) checked @endif> Hoàn thành</label>
